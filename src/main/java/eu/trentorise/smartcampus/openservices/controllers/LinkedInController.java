...
public class LinkedInController {
	...
	/**
	 * LoginTwitter starts oauth authentication with Twitter. First it searches
	 * in session for access and request token, because if they are not null
	 * then user is already logged in, otherwise it generates new request token.
	 * Then request token is saved in session and authorization url for login is
	 * created.
	 * 
	 * @param request
	 *            : instance of {@link HttpServletRequest}
	 * @return {@link ResponseObject} with redirected url, status (OK) and error
	...
	public ResponseObject loginTwitter(HttpServletRequest request) {
		...
		if (requestToken == null || accessToken == null) {
			...
			request.getSession()
					.setAttribute(OAUTH_REQUEST_TOKEN, requestToken);
	...
	/**
	 * CallbackTwitter is the url called back from LinkedIn after successful
	 * login from user. It gets request token from session. Then it gets access
	 * token from request token and a verifier, and stores it in session. Using
	 * this tokens, it is possible to retrieve user data. After that data is
	 * unmarshalled with XStream (xml format) and saved in db. If email is
	 * already in use, a security warning is thrown and user is redirected to
	 * home page without authentication and response status UNAUTHORIZED.
	 * Otherwise user is authenticated and cookie user is added, then user is
